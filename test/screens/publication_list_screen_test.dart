...
class _FakeOpenAlexService extends OpenAlexService {
  final List<Publication> citedByResults;
  int calls = 0;

  _FakeOpenAlexService({this.citedByResults = const []});

  @override
  Future<List<Publication>> fetchCitedBy(String workId, {int page = 1}) async {
    calls++;
    return citedByResults;
  }
...
Publication _pub(String id) => Publication(
      id: id,
      title: 'Paper $id',
...
Widget _buildScreen({
  ...
  return MultiProvider(
    providers: [
      ChangeNotifierProvider<PublicationListProvider>.value(value: listProvider),
...
void main() {
  ...
  group('ListType enum', () {
    test('contains all expected types', () {
      expect(ListType.values, containsAll([
        ListType.related,
        ListType.citedBy,
        ListType.references,
      ]));
