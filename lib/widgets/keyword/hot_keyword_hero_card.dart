    ...
    return Card(
      ...
      child: Padding(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(children: [
            const Text('Current Hot Keyword', style: TextStyle(fontWeight: FontWeight.w700)),
            const Spacer(),
            IconButton(
          ...
          Wrap(spacing: 10, runSpacing: 8, crossAxisAlignment: WrapCrossAlignment.center, children: [
            Text(keyword.name, style: Theme.of(context).textTheme.headlineSmall?.copyWith(fontWeight: FontWeight.bold)),
            KeywordStatusChip(status: keyword.status),
          ]),
          const SizedBox(height: 14),
          Text('Recent publications: ${format.format(keyword.currentPeriodCount)}'),
          Text('Previous period: ${format.format(keyword.previousPeriodCount)}'),
          Text('Growth: $growth', style: const TextStyle(fontWeight: FontWeight.w700, color: Colors.green)),
          const SizedBox(height: 6),
          Text(_summary(keyword.growthRate)),
          const SizedBox(height: 16),
          FilledButton(onPressed: onViewDetail, child: const Text('View Detail')),
        ]),
      ),
    );
  }
