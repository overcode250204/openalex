...
class KeywordDashboardTrendChart extends StatefulWidget {
  ...
  @override
  State<KeywordDashboardTrendChart> createState() => _KeywordDashboardTrendChartState();
}

class _KeywordDashboardTrendChartState extends State<KeywordDashboardTrendChart> {
  KeywordTrendRange _range = KeywordTrendRange.tenYears;
  static const _colors = [Color(0xFF2F6FB0), Color(0xFFE67E22), Color(0xFF16A085)];
  ...
  Widget build(BuildContext context) {
    ...
    for (final entry in widget.series.entries.take(3)) {
      ...
      filtered[entry.key] = KeywordTrendPoint.latestPoints(entry.value, limit: limit);
    }
    ...
    return Card(
      child: Padding(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(children: [
            Expanded(child: Text('Keyword Trend Comparison', style: Theme.of(context).textTheme.titleLarge?.copyWith(fontWeight: FontWeight.bold))),
            DropdownButton<KeywordTrendRange>(
              value: _range,
              items: const [
                DropdownMenuItem(value: KeywordTrendRange.fiveYears, child: Text('Last 5 years')),
                DropdownMenuItem(value: KeywordTrendRange.tenYears, child: Text('Last 10 years')),
                DropdownMenuItem(value: KeywordTrendRange.all, child: Text('All available years')),
              ],
            ...
            child: LineChart(LineChartData(
              ...
              titlesData: FlTitlesData(
                ...
                leftTitles: AxisTitles(sideTitles: SideTitles(
                  showTitles: true,
                  reservedSize: 42,
                  getTitlesWidget: (value, meta) => Text(Formatters.formatCompactAxis(value.toInt()), style: const TextStyle(fontSize: 10)),
                )),
              ...
              lineTouchData: LineTouchData(touchTooltipData: LineTouchTooltipData(
                getTooltipItems: (spots) => spots.map((spot) {
                  ...
                  return LineTooltipItem('$name\n${spot.x.toInt()}: ${spot.y.toInt()} publications', const TextStyle(color: Colors.white));
                }).toList(),
              )),
              lineBarsData: [
                for (var i = 0; i < filtered.length; i++)
                  LineChartBarData(
                    spots: filtered.values.elementAt(i).map((p) => FlSpot(p.year.toDouble(), p.count.toDouble())).toList(),
                    color: _colors[i],
                    barWidth: 3,
                    isCurved: true,
